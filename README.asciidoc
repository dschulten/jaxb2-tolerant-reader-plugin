...
To a certain extent it is possible to support multiple schema versions with a single JAXB bean, for details see the _src/it/xfinanz_ integration test and the section *Example bindings file*.
...
The https://github.com/dschulten/jaxb2-tolerant-reader-plugin/tree/master/tolerant-reader-plugin/src/it/xfinanz[it/xfinanz test project] shows how you can use the tolerant reader plugin together with Eclipselink MOXy in order to marshall two versions of a schema with a single set of JXB beans.

Its _bindings.xjb_ generates a synthetic getter/setter which handles a (contrived) incompatible change from a complete street address field to distinct fields for street name, house number and house number appendix. Note the `tr:set` element below:
    ...
    <tr:include>
      ...
      <tr:bean name="Postanschrift" properties="strasse hausnummer hausnummernzusatz postleitzahl ort staat adressierungszusatz">
        <tr:alias alias="strassekomplett">
          ...
          <!-- tr:set allows to parse setter input and assign the result to bean properties using Spring EL; supported from v0.5.2 -->
          <tr:set>
...
A second example in _src/it/xfinanz_ uses the tolerant-reader-plugin with the public schema https://www.xrepository.de/details/urn:xoev-de:agxfinanz:standard:xfinanz[XFinanz] created by German governments. It demonstrates how you can support two schema versions with a single JAXB bean, looking at a contrived scenario where the older schema version "had" a single field for road/houseNumber/houseNumberAppendix, whereas the current version uses three distinct fields.

In order to support two versions at once, we use Eclipselink MOXy during marshalling. Run the maven build and look at the _target/it/xfinanz/build.log_ to see the xml result. The key point is the ability of MOXy to customize the default namespace and to filter out JAXB bean properties which do not exist in the target schema version.
...
TIP: In situations where the schema uses a versioned `targetNamespace`, apply https://javaee.github.io/jaxb-v2/doc/user-guide/ch03.html#customization-of-schema-compilation-customizing-java-packages[custom java packages] to avoid having to fix lots of import statements for every version change. If you do this, the original versioned namespace will still be preserved during marshalling via the package definition in _package-info.java_.
Note that each version of a schema with versioned `targetNamespace` requires you to generate a different set of JAXB beans, i.e. your client still speaks only one version of the schema. In order to support multiple versions of a schema with a single set of JAXB beans, it is necessary to customize marshalling. Eclipselink MOXy allows this, as shown in the https://github.com/dschulten/jaxb2-tolerant-reader-plugin/tree/master/tolerant-reader-plugin/src/it/xfinanz[it/xfinanz test project]
