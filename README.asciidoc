...
List of expected bean properties as space-separated strings. Required properties are included automatically, i.e. you only need to define properties having minOccurs=0.
...
In Idea, create a run configuration as described above and configure it to *Use classpath of module: tolerant-reader-plugin*. In order to adjust the classpath to apply the xjc-classinfo-patch, choose *Open Module Settings* for the tolerant-reader-plugin module while it is highlighted (hit F4). On the *Dependencies* tab, hit the green + icon on the right hand side and choose *Module Dependency...* to add xjc-classinfo-patch. Then select xjc-classinfo-patch in the dependencies list and hit the up arrow icon until it is at the top of all dependencies.
