...
==== tr:alias element
Describes zero or more properties which should be generated with an alias name, one tr:alias element per property. The generated property will be renamed either to the content of the alias element or to the value of the alias attribute of the alias element. The property you want to rename is given with the property attribute (see below).
...
As an alternative, it is also valid to define the alias name with an alias attribute:
...
NOTE: The TolerantReaderPlugin cannot determine this type automatically because at the time of schema compilation the class of an `XmlAdapter` implementation cannot be available, since the `XmlAdapter` implementation requires the JAXB type.   
